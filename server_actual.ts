...
app.post('/api/powerbi/workspace/:workspaceId/datasets/takeover', async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const datasetId of datasetIds) {
      const url = `${config.apiBaseUrl}/v1.0/myorg/groups/${workspaceId}/datasets/${datasetId}/Default.TakeOver`;
      console.log(`\n[PBI API Call] POST ${url}`);
      ...
      if (!resp.ok) {
        ...
        results.push({ datasetId, success: false, error: respData?.error?.message || resp.statusText });
      } else {
        results.push({ datasetId, success: true });
      }
    }

    recordLog('Take Over Datasets', 'POST', `/api/powerbi/workspace/${workspaceId}/datasets/takeover`, 200, Date.now() - startTime, { datasetIds }, { results });

    const failed = results.filter(r => !r.success);
    if (failed.length > 0) {
      res.status(207).json({ success: true, message: 'Some takeovers failed', results });
    } else {
      res.json({ success: true, message: 'All datasets taken over successfully', results });
    }
