      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1 pl-8">
            Executes <code className="bg-slate-800 border border-slate-700 px-1 py-0.5 rounded text-amber-500 font-mono text-[11px]">POST /v1.0/myorg/pipelines/{'{pipelineId}'}/deploy</code> and actively polls <code className="bg-slate-800 border border-slate-700 px-1 py-0.5 rounded text-amber-500 font-mono text-[11px]">GET /operations/{'{operationId}'}</code> until status reports <span className="font-semibold text-emerald-500">Succeeded</span>.
          </p>
        </div>
      ...
      <div className="mt-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-3.5">
            <span className="text-[10px] uppercase font-bold text-slate-500">Promoted Scope</span>
            <p className="text-xs font-semibold text-slate-200 mt-1">
              {selectedReportsCount} Report(s) &bull; {selectedDatasetsCount} Dataset(s)
            </p>
            <span className="text-[11px] text-slate-400">Stage Order 1 → Order 2</span>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-lg p-3.5">
            <span className="text-[10px] uppercase font-bold text-slate-500">Safety &amp; Overwrite Options</span>
            ...
            <span className="text-[11px] text-slate-400">allowCreateArtifact: true, allowOverwrite: true</span>
          </div>

          <div className="bg-slate-900 border border-slate-800 rounded-lg p-3.5">
            <span className="text-[10px] uppercase font-bold text-slate-500">Polling State</span>
            <p className="text-xs font-semibold text-slate-200 mt-1 flex items-center gap-1.5">
              {isRunning ? (
                <>
                ...
                <span className="text-slate-400">Ready to Trigger</span>
              )}
            </p>
            <span className="text-[11px] text-slate-400">Waits for 200 Succeeded from Microsoft</span>
          </div>
        </div>

        {/* Release Note Input */}
        <div>
          <label htmlFor="deployment-note" className="block text-xs font-semibold text-slate-400 mb-1">
          ...
          <input
            ...
            className="w-full px-3 py-2 text-xs border border-slate-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-amber-500 bg-slate-950 text-slate-200"
            disabled={isRunning}
          />
        </div>
        ...
        {operation && (
          <div className={`p-4 rounded-xl border transition-all ${
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs">
              <div>
                <span className="text-slate-500 text-[10px]">Operation ID:</span>
                <p className="font-mono text-[11px] text-slate-300 break-all">{operation.id}</p>
              </div>
              <div>
                <span className="text-slate-500 text-[10px]">Execution Started:</span>
                <p className="text-slate-300">
                  {operation.executionStartTime ? new Date(operation.executionStartTime).toLocaleTimeString() : 'Just now'}
                </p>
              </div>
              <div>
                <span className="text-slate-500 text-[10px]">Execution Completed:</span>
                <p className="text-slate-300">
